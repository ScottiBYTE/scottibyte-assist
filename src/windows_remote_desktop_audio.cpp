...
bool RemoteDesktopAudio::startSender()
{
    ...
    /*
     * WASAPI loopback records the audio being rendered
     * through the default Windows output endpoint.
     *
     * Keep the transport format identical to Linux:
     *
     *   48 kHz stereo
     *   Opus 128 kbps
     *   20 ms frames
    ...
    if (error != nullptr) {
        emit errorOccurred(
            QStringLiteral(
                "Could not create Windows desktop "
                "audio sender: ") +
            errorText(error));
    ...
    const QString &outputNode)
{
    ...
    /*
     * Windows audio-device enumeration stores the WASAPI
     * endpoint ID. If the user selected one, send playback
     * to that endpoint. An empty value uses the default
    ...
    if (
        !startPipeline(
            receiverPipeline_,
            QStringLiteral(
                "Windows desktop audio receiver"))
    ) {
        stopReceiver();
        return false;
    }
